...
class Solution {
    ...
    public static long findPages(int n, int[] nums, int m) {
        // Your code here
        int start = 0 ;//initial start we want max element of entire array
        int end = 0 ;// we want total sum of each element

        //At start store the max element of array
        //At end strore the sum of entire array
        for (int i=0; i<nums.length ; i++)
        {
            start = Math.max(start,nums[i]);
            end = end + nums[i];
        }
        //if no of books are 4 and students are more than 4 , then question condition is not satisfied, 
        if(m > nums.length)
        {
            return -1;
        }
        while(start < end ) //here <= will not come because we want cause we want loop to break when start = end 
        {
            ...
            //search in left side of mid element
            if(pieces <= m) {
                end = mid;       
            }
            //searcing in right side of mid element
            else  {
                 start = mid+1;               
            }
        }
